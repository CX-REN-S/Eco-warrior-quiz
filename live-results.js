...
async function fetchGoogleSheets() {
  if (!PLEDGES_CSV_URL || PLEDGES_CSV_URL.includes('PASTE_PUBLIC')) {
...
function parseCSV(text) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const pledge = cells[pledgeIdx] || '';
    const count  = parseInt(cells[countIdx], 10);
    if (!pledge || SKIP_PLEDGES.has(pledge.toLowerCase())) continue;
...
function renderCloud(entries, source, errorMsg) {
  ...
  entries.forEach((e, i) => {
    ...
    const fontSize = 0.85 + pct * 1.65; // 0.85rem–2.5rem
    const opacity  = 0.65 + pct * 0.35;
    const color    = PLEDGE_COLORS[i % PLEDGE_COLORS.length];

    const word = document.createElement('div');
    word.className = 'cloud-word';
    word.style.cssText = `font-size:${fontSize}rem;color:${color};opacity:${opacity};animation-delay:${i * 0.08}s`;
    word.textContent = e.pledge;
    cloudEl.appendChild(word);
  });

  // Breakdown
  breakdownEl.innerHTML = '<h3>Pledges</h3>';
  [...entries].sort((a, b) => b.count - a.count).forEach((e, i) => {
    const color = PLEDGE_COLORS[i % PLEDGE_COLORS.length];
    const row = document.createElement('div');
    row.className = 'breakdown-row';
    row.innerHTML = `
      <span class="breakdown-name">${e.pledge}</span>
      <span class="breakdown-bar-wrap">
        <span class="breakdown-bar" style="width:${Math.round(e.count/maxCount*100)}%;background:${color}"></span>
      </span>
